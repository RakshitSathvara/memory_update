...
class BadeliSerdiReportViewModel(private val repository: BadeliSerdiReportRepository) : ViewModel() {
    ...
    val badeliSerdiReportResponse = MutableLiveData<BadeliSerdiReportResponse>()
    ...
    fun getBadeliSerdiReportApi(reportRequest: BadeliSerdiReportRequest) {
        progressObservable.postValue(true)
        viewModelScope.launch {
            try {
                val response = repository.badeliSerdiReport(reportRequest)
                progressObservable.postValue(false)
                if (response.isSuccessful) {
                    if (response.body()!!.response.equals(Constants.SUCCESS)) {
                        badeliSerdiReportResponse.postValue(response.body())
                    } else {
                        errorMessage.postValue(response.body()!!.message.toString())
                    }
                } else {
