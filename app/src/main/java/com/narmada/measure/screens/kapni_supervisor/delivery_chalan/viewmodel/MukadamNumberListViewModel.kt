...
class MukadamNumberListViewModel(private val repository: DeliveryChalanRepository) : ViewModel() {
    ...
    val mukadamNumberListResponse = MutableLiveData<MukadamNumberListResponse>()

    fun getMukadamNumberListApi(workingYear: String, search: String) {
        ...
        viewModelScope.launch {
            try {
                val response = repository.getMukadamNumberList(requestMap)
                val result: MukadamNumberListResponse? = response.body()
                if (response.isSuccessful && result != null) {
                    if (result.response.equals(Constants.SUCCESS)) {
                        mukadamNumberListResponse.postValue(result!!)
...
//                            errorMessage.postValue(errorCommonResponse.message.toString())
                        ...
                        mukadamNumberListResponse.postValue(MukadamNumberListResponse())
                    }
                    isLastPage = true
                    paginationProgressObservable.postValue(false)
                }
