...
class SabhasadMahitiViewModel(private val repository: SabhasadMahitiRepository) : ViewModel() {
    ...
    val sabhasadMahitiResponse = MutableLiveData<SabhasadMahitiResponse>()
    ...
    fun getSabhasadMahitiApi(reportRequest: SabhasadMahitiRequest) {
        progressObservable.postValue(true)
        viewModelScope.launch {
            try {
                val response = repository.sabhasadMahitiReport(reportRequest.village_id.toString())
                progressObservable.postValue(false)
                if (response.isSuccessful) {
                    if (response.body()!!.response.equals(Constants.SUCCESS)) {
                        sabhasadMahitiResponse.postValue(response.body())
                    } else {
                        errorMessage.postValue(response.body()!!.message.toString())
                    }
                } else {
